...
abstract class PostSet extends postsetInterface {
  ...
  def descendingByLikes: PostList =
    Try(mostLiked) match {
      case Success(max) => new Cons(max, remove(max).descendingByLikes)
      case Failure(e) => Nil
    }
...
class NonEmpty(elem: Post, left: PostSet, right: PostSet) extends PostSet {
  ...
  def mostLiked: Post = {
    def maxLikes(it: Post, other: Post): Post =
      if (it.likes > other.likes) it else other

    def getMostLiked(s: PostSet): Post =
      Try(s.mostLiked) match {
        case Success(v) => v
        case Failure(e) => new Post("nil", "nil", -1)
      }
...
object GoogleVsApple {
  ...
  lazy val googleposts: PostSet = PostReader.allposts.filter(p => googleFilter(p))
  lazy val appleposts: PostSet = PostReader.allposts.filter(p => appleFilter(p))
