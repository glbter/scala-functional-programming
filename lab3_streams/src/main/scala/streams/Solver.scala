...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.to(LazyList).map(p => (p._1, p._2 :: history))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val more = for {
      ...
      next <- newNeighborsOnly(
        (neighborsWithHistory _).tupled(block), explored
      )

      if (explored.count(b => done(b)) != 1)
    } yield next
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(LazyList.cons((startBlock, Nil), LazyList.empty), Set.empty)
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.map(p => p._2).head
}
