...
class BloxorzSuite extends munit.FunSuite {
  ...
  test("neighbors with history got level 1") {
    new Level1 {
      val got: Set[(Block, List[Move])] = Set.from(
        neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), Left::Up::Nil))
      val expect: Set[(Block, List[Move])] = Set(
